...
func TestWithPassthroughPaths(t *testing.T) {
	...
	t.Run("invalid passthrough options", func(t *testing.T) {
		// Duplicated /api.
		_, err := injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithPassthroughPaths([]string{"/api1", "/api2/something", "/api1"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// Wrong format, params in path.
		_, err = injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithPassthroughPaths([]string{"/api1?args=1", "/api1"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// / is not allowed.
		_, err = injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithPassthroughPaths([]string{"/", "/api2/something", "/api1"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// "" is not allowed.
		_, err = injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithPassthroughPaths([]string{"/api1", "/api2/something", "", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// Duplication with existing enforced path is not allowed.
		_, err = injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithPassthroughPaths([]string{"/api1", "/api2/something", "/federate", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// Duplication with existing enforced path is not allowed.
		...
		// Duplication with existing enforced path is not allowed.
		_, err = injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithPassthroughPaths([]string{"/api1", "/api2/something", "/federate/some", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// api4 is not valid URL path (does not start with /)
		_, err = injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithPassthroughPaths([]string{"/api1", "/api2/something", "api4", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// api4/ is not valid URL path (does not start with /)
		...
		_, err = injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithPassthroughPaths([]string{"/api1", "/api2/something", "api4/something", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
	})
	r, err := injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithPassthroughPaths([]string{"/api1", "/api2/something", "/graph/"}))
	if err != nil {
...
func TestClusterMatch(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			matches:  []string{`{job="prometheus",__name__=~"job:.*",cluster=~"default2",namespace=~"default3"}`},
			expCode:  http.StatusOK,
			expMatch: []string{`{job="prometheus",__name__=~"job:.*",cluster=~"default"}`},
			expBody:  okResponse,
		},
	} {
		for _, u := range []string{
			...
			t.Run(fmt.Sprintf("%s?match[]=%s", u, strings.Join(tc.matches, "&")), func(t *testing.T) {
				...
				r, err := injectproxy.NewRoutes(m.url, proxyLabel2, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithEnabledLabelsAPI())
				if err != nil {
					t.Fatalf("unexpected error: %v", err)
				}
				...
				//req2 := checkQueryHandler("", matchersParam, tc.expMatch...)
				//kvs, err := url.ParseQuery(u.String())
				//fmt.Println(kvs)
				...
				matcher := &labels.Matcher{
					...
					Value: labelValuesToRegexpString([]string{"something"}),
				}
				if err := injectMatcher(q, matcher); err != nil {
					return
				}
				ms, _ := ParseAuthorizations(q)
				fmt.Println("TEST METRIC SELECTOR: ", ms)
				fmt.Println("TEST QUERY: ", u.String())
	...
	for _, tc := range []struct {
		...
		for _, u := range []string{
			...
			t.Run(fmt.Sprintf("%s?match[]=%s", u, strings.Join(tc.matches, "&")), func(t *testing.T) {
				...
				r, err := injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}, injectproxy.WithEnabledLabelsAPI())
				if err != nil {
					t.Fatalf("unexpected error: %v", err)
				}
	...
	for _, tc := range []struct {
		...
		for _, endpoint := range []string{"series"} {
			t.Run(endpoint+"/"+strings.ReplaceAll(tc.name, " ", "_"), func(t *testing.T) {
				...
				r, err := injectproxy.NewRoutes(m.url, proxyLabel, injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel})
				if err != nil {
					t.Fatalf("unexpected error: %v", err)
				}
	...
	for _, tc := range []struct {
		...
		for _, endpoint := range []string{"query", "query_range", "query_exemplars"} {
			t.Run(endpoint+"/"+strings.ReplaceAll(tc.name, " ", "_"), func(t *testing.T) {
				...
				var labelEnforcer injectproxy.ExtractLabeler
				if len(tc.staticLabelVal) > 0 {
					labelEnforcer = injectproxy.StaticLabelEnforcer(tc.staticLabelVal)
				} else if tc.headerName != "" {
					labelEnforcer = injectproxy.HTTPHeaderEnforcer{Name: tc.headerName}
				} else if tc.queryParam != "" {
					labelEnforcer = injectproxy.HTTPFormEnforcer{ParameterName: tc.queryParam}
				} else {
					labelEnforcer = injectproxy.HTTPFormEnforcer{ParameterName: proxyLabel}
				}

				r, err := injectproxy.NewRoutes(m.url, proxyLabel, labelEnforcer, opts...)
				if err != nil {
					t.Fatalf("unexpected error: %v", err)
				}
